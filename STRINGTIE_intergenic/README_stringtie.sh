...
## ./gff2gtf.pl < saccharomyces_cerevisiae_R64-4-1_20230830.gff > saccharomyces_cerevisiae_R64-4-1_20230830_with_repeats.gtf

# Extract LTR coords from SGD gff file
grep LTR saccharomyces_cerevisiae_R64-4-1_20230830.gff|perl -ne '@x=split /\t/;$x[3]--;$name=$1 if $x[8]=~m/Name=(\S+?);/ ;print "$x[0]\t$x[3]\t$x[4]\t$name\t.\t$x[6]\n"' > LTR.bed
...
for i in {1..12} {31..42}; do
	...
	stringtie ../data/BAM/AY${i}_*.sorted.dedup.bam --rf -o AY${i}.gtf -G Saccharomyces_cerevisiae.R64-1-1.106.gtf -p 4
	...
	cat AY${i}.no_LTR.gtf | perl -e 'open I, "AY1.no_LTR.ref_genes.ids"; while(<I>){chomp;$h{$_}++}; close I; while(<>){$geneid="none"; $geneid=$1 if m/gene_id\s"(\S+?)";/ ; print "$_" unless $h{$geneid} > 0}' > AY${i}.no_LTR.NO_ref_genes.gtf

	# Generate stranded bigwig file 
	my_message "Generate stranded bigwig file"
	igvtools count --strands first -z 7 -w 15  ../data/BAM/AY${i}_*.sorted.dedup.bam AY${i}.wig Saccharomyces_cerevisiae.R64-1-1.106.fa
	...
	./wigToBigWig AY${i}.both.wig chromosome_sizes.txt AY${i}.both.bw
	...
	bedtools intersect -a AY${i}.no_LTR.NO_ref_genes.gtf -b Saccharomyces_cerevisiae.R64-1-1.106.gtf -wa -v -s > AY${i}.no_overlap.no_LTR.NO_ref_genes.gtf
...
./search_overlap_promoter.pl -i aymerge.no_LTR.NO_ref_genes.gtf -p ./ScerYPDconsensusClusters.gff -w 100 |perl -ne 'print "$1\n" if m/gene_id\s"(MSTRG.\d+?)";/ ' > aymerge.no_LTR.NO_ref_genes_with_PROMOTERS_100bp_window.txt
...
bedtools intersect -a aymerge.no_LTR.NO_ref_genes.gtf -b ./Nagalakshmi_2008_5_3_UTRs_V64.bed -wa -s -f 0.1 -r -wb |grep -v exon| perl -ne '@x=split /\t/;print "$1\t$x[12]\n" if m/(MSTRG.\d+)/' > aymerge.no_LTR.NO_ref_genes_Overlap_Nagalakshmi_2008_5_3_UTRs_V64.txt
